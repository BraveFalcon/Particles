...
def get_fig_kin_energy(ts, energies):
    ...
    mean_energy = np.mean(energies[len(energies) // 2:])
    std_energy = np.std(energies[len(energies) // 2:])

    ax.plot(ts, energies / mean_energy, 'ro', ms=2)
    ax.plot([ts[0], ts[-1]], [1 + std_energy / mean_energy] * 2, 'g-', lw=2)
    ax.plot([ts[0], ts[-1]], [1 - std_energy / mean_energy] * 2, 'g-', lw=2)
